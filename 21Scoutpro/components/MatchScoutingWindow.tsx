...
interface MatchEvent {
  ...
  /** ID do goleiro selecionado quando método é "Defesa de goleiro linha" */
  goalkeeperId?: string;
  /** Nome do goleiro selecionado quando método é "Defesa de goleiro linha" */
...
function postMatchEventLogToMatchEvents(log: PostMatchEvent[], players: Player[]): MatchEvent[] {
  ...
  return log.map(pe => {
    const timeSeconds = parseMMSSToSeconds(pe.time);
    ...
    if (pe.isOpponentGoal === true) event.isOpponentGoal = true;
    ...
    if (pe.goalkeeperId) {
      event.goalkeeperId = String(pe.goalkeeperId).trim();
      event.goalkeeperName = pe.goalkeeperName ?? playerById.get(event.goalkeeperId)?.name;
    }
...
export const MatchScoutingWindow: React.FC<MatchScoutingWindowProps> = ({
  ...
  const [pendingGoalPeriod, setPendingGoalPeriod] = useState<'1T' | '2T' | null>(null); // Período do gol (editável no confirm)
  const [goalConfirmEditingTime, setGoalConfirmEditingTime] = useState<boolean>(false); // Toggle editar tempo no popup confirmar
  const [goalStep, setGoalStep] = useState<'team' | 'author' | 'assist' | 'method' | 'goalkeeper' | 'confirm' | null>(null); // Fluxo inline do gol
  ...
  const [pendingGoalkeeperId, setPendingGoalkeeperId] = useState<string | null>(null); // ID do goleiro selecionado no método Defesa de goleiro linha
  ...
  const draftStorageKeyRef = useRef<string>('');
  ...
  // Novo fluxo: Ação → Detalhes (popup) → [Passe Errado: Gerou transição?] → Jogador/Goleiro (popup) → Tempo (popup se necessário)
  ...
  const startActionFlow = (action: string) => {
    ...
    // Escanteio e Lateral: registrar com um clique, sem confirmação de jogador (usa selecionado ou primeiro ativo)
    if (action === 'lateral' || action === 'corner') {
      const pid = selectedPlayerId ?? (activePlayers.length > 0 ? String(activePlayers[0].id).trim() : null);
      if (pid) {
        if (needsTimePopup()) {
          setActionFlow({ ...flow, step: 'time', selectedPlayerId: pid });
        } else {
    ...
    if (step === 'player' && selectedPlayerId) {
      if (needsTimePopup()) {
        setActionFlow({ ...flow, step: 'time', selectedPlayerId });
      } else {
        executeActionFlow(flow as NonNullable<typeof actionFlow>, selectedPlayerId);
        setActionFlow(null);
        setSelectedAction(null);
      }
      return;
  ...
  const advanceActionFlowToPlayer = (details: string | null, extra?: { cardType?: 'yellow' | 'secondYellow' | 'red'; foulTeam?: 'for' | 'against'; zone?: LateralResult; wrongPassTransition?: boolean }) => {
    // Defesa: após escolher fácil/difícil, selecionar goleiro em popup dedicado.
    if (actionFlow?.action === 'save') {
      const nextFlow = actionFlow ? { ...actionFlow, step: 'goalkeeper' as const, details, ...extra } : null;
      setActionFlow(nextFlow);
      return;
    }

    // Passe: usar jogador da aba lateral, sem passo de confirmação de jogador
    ...
    // Lateral e escanteio: usar jogador da aba lateral, sem passo de confirmação
    if (actionFlow?.action === 'lateral' || actionFlow?.action === 'corner') {
    ...
    if (nextFlow && selectedPlayerId) {
      if (needsTimePopup()) {
        setActionFlow({ ...nextFlow, step: 'time', selectedPlayerId });
      } else {
        executeActionFlow(nextFlow, selectedPlayerId);
        cancelActionFlow();
      }
      return;
    }
  ...
  const executeActionFlow = (
    ...
    const time = timeOverride ?? getTimeForEvent() ?? matchTime;
    ...
    const run = () => {
      switch (flow.action) {
        ...
        case 'shot':
          ...
          break;
        case 'foul':
          handleRegisterFoul(flow.foulTeam ?? 'for', playerId, time, period);
          break;
        case 'tackle':
          handleRegisterTackle(flow.details as 'withBall' | 'withoutBall' | 'counter', playerId, time, period);
          break;
        case 'card':
          handleRegisterCard(flow.cardType ?? 'yellow', playerId, time, period);
          break;
        case 'save':
          handleRegisterSave(flow.details as 'simple' | 'hard', playerId, time, period);
          break;
        case 'lateral':
          handleRegisterLateral(flow.zone, playerId, time, period);
          break;
        case 'corner':
          handleRegisterCorner(flow.zone, playerId, time, period);
          break;
        case 'block':
          handleRegisterBlock(playerId, time, period);
          break;
        default:
          break;
      }
  ...
  useEffect(() => {
    const idStr = match?.id != null ? String(match.id).trim() : '';
    draftStorageKeyRef.current = idStr ? `scout21_match_draft_${idStr}` : '';
  }, [match?.id]);

  // Carregar eventos da partida ao abrir (postmatch ou partida incompleta com log salvo)
  useEffect(() => {
    if (!isOpen) return;
    let logSource = match.postMatchEventLog;
    if ((!logSource || logSource.length === 0) && draftStorageKeyRef.current) {
      try {
        const rawDraft = localStorage.getItem(draftStorageKeyRef.current);
        if (rawDraft) {
          const draftMatch = JSON.parse(rawDraft) as MatchRecord;
          if (draftMatch?.postMatchEventLog?.length) {
            logSource = draftMatch.postMatchEventLog;
          }
          if (!isPostmatch) {
            if (draftMatch?.lineup?.players?.length) setLineupPlayers(draftMatch.lineup.players);
            if (draftMatch?.lineup?.bench?.length) setBenchPlayers(draftMatch.lineup.bench);
            if (draftMatch?.lineup?.ballPossessionStart) setBallPossessionStart(draftMatch.lineup.ballPossessionStart);
            if (draftMatch?.substitutionHistory?.length) setSubstitutionHistory(draftMatch.substitutionHistory);
          }
        }
      } catch (_) {}
    }
    if (!logSource?.length) return;
    const converted = postMatchEventLogToMatchEvents(logSource, players);
  ...
  }, [isOpen, match?.id, match?.postMatchEventLog, match?.lineup, match?.substitutionHistory, players, isPostmatch]);
  ...
  const convertMatchEventsToMatchRecord = (events: MatchEvent[]): MatchRecord => {
    ...
    for (const e of events) {
      ...
      if (action === 'goal' && e.isOpponentGoal && !e.playerId) {
        ...
        if (e.goalkeeperId) {
          postEventFallback.goalkeeperId = String(e.goalkeeperId).trim();
          if (e.goalkeeperName) postEventFallback.goalkeeperName = e.goalkeeperName;
        }
      ...
      if (action === 'goal') {
        ...
        if (e.goalkeeperId) {
          postEvent.goalkeeperId = String(e.goalkeeperId).trim();
          if (e.goalkeeperName) postEvent.goalkeeperName = e.goalkeeperName;
        }
      }
  ...
  const buildMatchSnapshot = (status: 'em_andamento' | 'encerrado', eventsOverride?: MatchEvent[]): MatchRecord => {
    const sourceEvents = eventsOverride ?? matchEvents;
    const savedMatch = convertMatchEventsToMatchRecord(sourceEvents);
  ...
  const saveSilently = async (eventsOverride?: MatchEvent[]) => {
    if (!onSave || !isOpen || autosaveSkipRef.current) return;
    if (!isPostmatch && !isMatchStarted) return;
    const sourceEvents = eventsOverride ?? matchEvents;
    if (sourceEvents.length === 0) return;

    const snapshot = buildMatchSnapshot('em_andamento', sourceEvents);
  ...
  const goalkeepersInMatch = useMemo(() => {
    const byId = new Map(players.map((p) => [String(p.id).trim(), p]));
    const ids = new Set<string>();

    const maybeAddGoalkeeper = (playerId?: string | null) => {
      if (!playerId) return;
      const pid = String(playerId).trim();
      const p = byId.get(pid);
      if (!p) return;
      if (p.position === 'Goleiro' || pid === currentGoalkeeperId) {
        ids.add(pid);
      }
    };

    // Goleiros que entraram: escalação inicial + substituições + goleiro atual.
    lineupPlayers.forEach((id) => maybeAddGoalkeeper(id));
    substitutionHistory.forEach((sub) => {
      maybeAddGoalkeeper(sub.playerInId);
      maybeAddGoalkeeper(sub.playerOutId);
    });
    maybeAddGoalkeeper(currentGoalkeeperId);

    // Fallback: se não achou ninguém pela participação, usar goleiros ativos na coleta
    if (ids.size === 0) {
  ...
  }, [players, lineupPlayers, substitutionHistory, currentGoalkeeperId, activePlayers]);
  ...
  const handleEndCollection = async () => {
    ...
    if (draftStorageKeyRef.current) {
      localStorage.removeItem(draftStorageKeyRef.current);
    }
    onClose();
  };
  ...
  useEffect(() => {
    ...
    if (autosaveSkipRef.current) return;
    if (matchEvents.length === 0) return;
    if (!draftStorageKeyRef.current) return;
    try {
      const draftSnapshot = buildMatchSnapshot('em_andamento');
      localStorage.setItem(draftStorageKeyRef.current, JSON.stringify(draftSnapshot));
    } catch (_) {}
  }, [isOpen, matchEvents, lineupPlayers, benchPlayers, substitutionHistory, possessionSecondsWith, possessionSecondsWithout]);
  ...
  // Selecionar ação — novo fluxo: ação → detalhes (popup) → jogador (popup) → tempo (popup se necessário)
  ...
  const handleRegisterTackle = (result: 'withBall' | 'withoutBall' | 'counter', playerIdOverride?: string, timeOverride?: number, periodOverride?: '1T' | '2T') => {
    ...
    const newEvent: MatchEvent = {
      ...
      time: timeOverride ?? (getTimeForEvent() ?? matchTime),
      period: periodOverride ?? currentPeriod,
      result,
      tipo,
      subtipo,
    };
    
    const nextEvents = [...matchEvents, newEvent];
    setMatchEvents(nextEvents);
    if (draftStorageKeyRef.current) {
      try {
        const fastDraft = buildMatchSnapshot('em_andamento', nextEvents);
        localStorage.setItem(draftStorageKeyRef.current, JSON.stringify(fastDraft));
      } catch (_) {}
    }
    // Evento crítico: tenta persistir imediatamente para sobreviver refresh rápido.
    void saveSilently(nextEvents);
  ...
  const handleRegisterGoal = (
    goalType: 'normal' | 'contra',
    isOpponent: boolean = false,
    playerId: string | null = null,
    goalMethod?: string | null,
    goalTimeOverride?: number | null,
    goalPeriodOverride?: '1T' | '2T' | null,
    ...
    const goalkeeper = goalkeeperId ? players.find(p => String(p.id).trim() === String(goalkeeperId).trim()) : null;
    ...
    const newEvent: MatchEvent = {
      ...
      ...(goalkeeperId && { goalkeeperId, goalkeeperName: goalkeeperName ?? goalkeeper?.name }),
    };
  ...
  const handleSelectGoalMethod = (method: string) => {
    setPendingGoalMethod(method);
    if (method === 'Defesa de goleiro linha') {
      setPendingGoalkeeperId(null);
      setGoalStep('goalkeeper');
      return;
    }
    setGoalStep('confirm');
  };
    ...
    <div className={`fixed z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center animate-fade-in overflow-hidden p-0 ${
      ...
      <div className="w-full h-full min-h-0 bg-black flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 flex gap-2 p-2 overflow-hidden min-h-0 min-w-0">
          ...
          <div className={`w-52 min-w-[7rem] max-w-[14rem] rounded-lg p-2 flex flex-col border-2 shrink-0 min-h-0 bg-black ${actionFlow?.step === 'player' ? 'border-[#00f0ff]/70' : goalStep === 'author' ? 'border-green-500/70' : goalStep === 'assist' ? 'border-amber-500/70' : 'border-zinc-800'}`}>
            <h3 className="text-white font-bold uppercase text-sm mb-2 text-center shrink-0">
              {goalStep === 'author' ? 'AUTOR DO GOL' : goalStep === 'assist' ? 'ASSISTÊNCIA' : 'SELECIONAR JOGADOR'}
            </h3>
            {goalStep === 'author' && (
              <>
                <p className="text-green-400 text-[10px] font-bold uppercase text-center mb-2 shrink-0">Selecione na lista abaixo</p>
                <button type="button" onClick={() => { setGoalStep('team'); setPendingGoalPlayerId(null); setPendingAssistPlayerId(null); setPendingGoalType(null); setPendingGoalMethod(null); setPendingGoalkeeperId(null); setGoalConfirmEditingTime(false); }} className="mb-2 w-full py-1.5 bg-zinc-700 hover:bg-zinc-600 text-zinc-300 font-bold uppercase text-[10px] rounded-lg border border-zinc-600 transition-colors shrink-0">Voltar</button>
              </>
            )}
            {goalStep === 'assist' && (
              <>
                <p className="text-amber-400 text-[10px] font-bold uppercase text-center mb-2 shrink-0">Quem deu a assistência?</p>
                <button type="button" onClick={() => { setGoalStep('author'); setPendingAssistPlayerId(null); }} className="mb-2 w-full py-1.5 bg-zinc-700 hover:bg-zinc-600 text-zinc-300 font-bold uppercase text-[10px] rounded-lg border border-zinc-600 transition-colors shrink-0">Voltar</button>
              </>
            )}
            {actionFlow?.step === 'player' && !goalStep && (
              <p className="text-[#00f0ff] text-[10px] font-bold uppercase text-center mb-2 shrink-0">Selecione na lista abaixo</p>
            )}
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (goalStep === 'assist') {
                            ...
                            if (pendingGoalMethod) setGoalStep('confirm'); else { setGoalStep('method'); setPendingGoalMethod(null); }
                            return;
                          }
                          if (!isMatchStarted) return;
                          if (actionFlow?.step === 'player' && actionFlow?.action) {
                            if (needsTimePopup()) {
                              setActionFlow(prev => prev ? { ...prev, step: 'time' as const, selectedPlayerId: clickedPlayerId, details: prev.details || undefined, wrongPassTransition: prev.wrongPassTransition } : null);
                            } else {
                        ...
                        className={`w-full rounded-lg p-2 text-left transition-all ${
                          goalStep === 'author'
                            ? 'bg-green-500/15 border-2 border-green-500/80 hover:border-green-500 hover:bg-green-500/20 cursor-pointer'
                            : goalStep === 'assist'
                            ? 'bg-amber-500/15 border-2 border-amber-500/80 hover:border-amber-500 hover:bg-amber-500/20 cursor-pointer'
                            : !isMatchStarted
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        // Se o fluxo de ação está aguardando jogador, usar clique da aba lateral (sem pop-up)
                        if (actionFlow?.step === 'player' && actionFlow?.action) {
                          if (needsTimePopup()) {
                            setActionFlow(prev => prev ? { ...prev, step: 'time' as const, selectedPlayerId: clickedPlayerId, details: prev.details || undefined, wrongPassTransition: prev.wrongPassTransition } : null);
                          } else {
                      ...
                      className={`w-full rounded-lg p-1.5 text-left transition-all ${
                        goalStep === 'author'
                          ...
                          ? 'bg-[#00f0ff]/15 border-2 border-[#00f0ff]/80 hover:border-[#00f0ff] hover:bg-[#00f0ff]/20'
                          : isSelected
                          ? 'bg-[#00f0ff]/20 border-2 border-[#00f0ff]'
                          : pendingPassEventId && String(player.id).trim() !== pendingPassSenderId
                          ? 'bg-yellow-500/20 border border-yellow-500 hover:border-yellow-400'
            ...
            {actionFlow?.step === 'player' && !goalStep && (
              <div className="mb-2 p-2 bg-[#00f0ff]/10 border border-[#00f0ff]/50 rounded-lg">
                <p className="text-[#00f0ff] text-xs font-bold text-center">
                  Selecione o jogador na aba lateral
                </p>
              </div>
            )}
            {/* Indicação quando aguarda autor do gol na aba lateral */}
            {goalStep === 'author' && (
              <div className="mb-2 p-2 bg-green-500/10 border border-green-500/50 rounded-lg">
                <p className="text-green-400 text-xs font-bold text-center">
                  Selecione o autor do gol na aba lateral
                </p>
              </div>
            )}
            {goalStep === 'assist' && (
              <div className="mb-2 p-2 bg-amber-500/10 border border-amber-500/50 rounded-lg flex flex-col gap-2">
                <p className="text-amber-400 text-xs font-bold text-center">
                  Selecione quem deu a assistência na aba lateral ou
                </p>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="w-full py-2.5 bg-zinc-700 hover:bg-zinc-600 border-2 border-zinc-600 text-zinc-300 font-bold uppercase text-xs rounded-lg transition-colors"
                >
                  Sem assistência
                </button>
              </div>
            ...
            <div className="flex-1 border-2 border-zinc-800 rounded-lg p-2 flex flex-col min-h-0">
              ...
              {goalStep === 'team' && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-fade-in">
                  <div
                    ...
                    onClick={() => { setGoalStep(null); setPendingGoalTime(null); setPendingGoalPeriod(null); setPendingGoalkeeperId(null); setGoalConfirmEditingTime(false); }}
                    aria-hidden="true"
                  />
                  <div className="relative w-full max-w-md bg-zinc-950 border-2 border-green-500/40 rounded-2xl shadow-2xl shadow-green-500/10 overflow-hidden">
                    ...
                    <div className="p-4 flex flex-col gap-3">
                      ...
                      <button
                        ...
                        className="w-full px-4 py-4 bg-amber-500/20 border-2 border-amber-500 text-amber-400 font-bold uppercase text-sm rounded-xl hover:bg-amber-500/30 transition-colors"
                      >
                        Gol Contra
                      </button>
                      <button
                      ...
                      <button
                        onClick={() => { setGoalStep(null); setPendingGoalTime(null); setPendingGoalPeriod(null); setPendingGoalMethod(null); setPendingGoalkeeperId(null); setPendingGoalType(null); setPendingGoalIsOpponent(false); setPendingGoalPlayerId(null); setPendingAssistPlayerId(null); setGoalConfirmEditingTime(false); }}
              ...
              {goalStep === 'method' && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-fade-in">
                  <div
                    className="absolute inset-0 bg-black/70 backdrop-blur-sm"
                    onClick={() => { setGoalStep((pendingGoalIsOpponent || pendingGoalType === 'contra') ? 'team' : 'assist'); setPendingGoalMethod(null); setPendingGoalkeeperId(null); }}
                    aria-hidden="true"
                  />
              ...
              {goalStep === 'goalkeeper' && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-fade-in">
                  <div
                    className="absolute inset-0 bg-black/70 backdrop-blur-sm"
                    onClick={() => { setGoalStep('method'); setPendingGoalkeeperId(null); }}
                    aria-hidden="true"
                  />
                  <div className="relative w-full max-w-lg bg-zinc-950 border-2 border-[#00f0ff]/40 rounded-2xl shadow-2xl shadow-[#00f0ff]/10 overflow-hidden">
                    <div className="p-4 border-b border-zinc-800 bg-gradient-to-r from-zinc-900 to-zinc-950">
                      ...
                      <p className="text-zinc-500 text-xs mt-1">
                        Escolha o goleiro que participou da jogada.
                      </p>
                    </div>
                    <div className="p-4 max-h-[70vh] overflow-y-auto space-y-2">
                      {goalkeepersInMatch.length > 0 ? goalkeepersInMatch.map((gk) => {
                        const gkId = String(gk.id).trim();
                        const isSelected = pendingGoalkeeperId === gkId;
                        return (
                          <button
                            key={gkId}
                            onClick={() => setPendingGoalkeeperId(gkId)}
                            className={`w-full rounded-xl border-2 p-3 text-left transition-colors ${
                              isSelected
                                ? 'bg-[#00f0ff]/20 border-[#00f0ff] text-white'
                                : 'bg-zinc-900 border-zinc-700 hover:border-[#00f0ff]/60 text-zinc-200'
                            }`}
                          >
                            <p className="font-bold text-sm">
                              {gk.jerseyNumber ? `#${gk.jerseyNumber} ` : ''}{gk.nickname?.trim() || gk.name}
                            </p>
                            <p className="text-[10px] uppercase tracking-wide text-zinc-500 mt-0.5">Goleiro participante</p>
                          </button>
                        );
                      }) : (
                        <div className="rounded-xl border border-zinc-700 bg-zinc-900 p-3 text-zinc-400 text-sm">
                          Nenhum goleiro participante encontrado.
                        </div>
                      )}
                    </div>
                    <div className="p-3 border-t border-zinc-800 bg-zinc-900/50 flex gap-2 justify-end">
                      <button
                        onClick={() => { setGoalStep('method'); setPendingGoalkeeperId(null); }}
                        className="flex items-center gap-1.5 px-4 py-2 bg-zinc-700 hover:bg-zinc-600 text-zinc-300 font-bold uppercase text-xs rounded-lg border border-zinc-600 transition-colors"
                      >
                      ...
                      <button
                        onClick={() => {
                          if (!pendingGoalkeeperId) return;
                          setGoalStep('confirm');
                        }}
                        disabled={!pendingGoalkeeperId}
                        className={`px-4 py-2 font-bold uppercase text-xs rounded-lg transition-colors border ${
                          pendingGoalkeeperId
                            ? 'bg-[#00f0ff]/20 hover:bg-[#00f0ff]/30 text-[#00f0ff] border-[#00f0ff]/60'
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-fade-in">
                  ...
                  <div className="relative w-full max-w-md bg-zinc-950 border-2 border-green-500/40 rounded-2xl shadow-2xl shadow-green-500/10 overflow-hidden">
                    ...
                    <div className="p-5 space-y-3">
                      ...
                      {!pendingGoalIsOpponent && pendingGoalType !== 'contra' && (
                        <div className="flex items-center gap-3 p-3 bg-zinc-900 rounded-xl border border-zinc-800">
                      ...
                      {pendingGoalMethod === 'Defesa de goleiro linha' && (
                        <div className="flex items-center gap-3 p-3 bg-zinc-900 rounded-xl border border-zinc-800">
                          <span className="text-zinc-500 text-xs font-bold uppercase w-20 shrink-0">Goleiro</span>
                          <span className="text-white text-sm font-bold">
                            {pendingGoalkeeperId
                              ? (() => {
                                  const gk = players.find((x) => String(x.id).trim() === pendingGoalkeeperId);
                                  return gk ? `${gk.jerseyNumber ? `#${gk.jerseyNumber} ` : ''}${gk.nickname || gk.name || '—'}` : '—';
                                })()
                              : '—'}
                          </span>
                        </div>
                    ...
                    <div className="p-4 border-t border-zinc-800 bg-zinc-900/50 flex gap-3">
                      <button
                        onClick={() => {
                          if (pendingGoalMethod === 'Defesa de goleiro linha' && !pendingGoalkeeperId) return;
                          ...
                          handleRegisterGoal(
                            ...
                            pendingGoalkeeperId ?? undefined,
                            pendingGoalkeeperId
                              ? (players.find((x) => String(x.id).trim() === pendingGoalkeeperId)?.name ?? undefined)
                              : undefined
                          );
                        }}
                        ...
                        disabled={pendingGoalMethod === 'Defesa de goleiro linha' && !pendingGoalkeeperId}
                      >
                        Confirmar Gol
                      </button>
                      <button
                  ...
                  <div className="relative w-full max-w-md bg-zinc-950 border-2 border-zinc-700 rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div className="p-4">
                      ...
                      {actionFlow.action === 'save' && (
                        <div className="space-y-3">
                          <p className="text-zinc-400 text-xs">Após escolher o tipo, selecione o goleiro no popup.</p>
                          <div className="grid grid-cols-2 gap-3">
                            <button onClick={() => { advanceActionFlowToPlayer('simple'); }} className="px-4 py-3 bg-purple-500/20 border border-purple-500 text-purple-400 font-medium uppercase text-xs rounded-lg hover:bg-purple-500/30 transition-colors">Defesa fácil</button>
                            <button onClick={() => { advanceActionFlowToPlayer('hard'); }} className="px-4 py-3 bg-purple-600/20 border border-purple-600 text-purple-300 font-medium uppercase text-xs rounded-lg hover:bg-purple-600/30 transition-colors">Defesa difícil</button>
                          </div>
                        </div>
                      )}
                    </div>
              ...
              {/* Popup seleção de goleiro para ação Defesa */}
              {actionFlow?.step === 'goalkeeper' && actionFlow.action === 'save' && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-fade-in">
                  ...
                  <div className="relative w-full max-w-lg bg-zinc-950 border-2 border-purple-500/40 rounded-2xl shadow-2xl shadow-purple-500/10 overflow-hidden">
                    <div className="p-4 border-b border-zinc-800 bg-gradient-to-r from-zinc-900 to-zinc-950">
                      <h3 className="text-purple-300 font-black uppercase text-sm tracking-wider flex items-center gap-2">
                        <Shield size={18} />
                        Selecionar goleiro
                      </h3>
                      <p className="text-zinc-500 text-xs mt-1">Escolha o goleiro que fez a defesa.</p>
                    </div>
                    <div className="p-4 max-h-[70vh] overflow-y-auto space-y-2">
                      {goalkeepersInMatch.length > 0 ? goalkeepersInMatch.map((gk) => {
                        const gkId = String(gk.id).trim();
                        const selected = actionFlow.selectedPlayerId === gkId;
                        return (
                          <button
                            key={gkId}
                            onClick={() => setActionFlow(prev => prev ? { ...prev, selectedPlayerId: gkId } : prev)}
                            className={`w-full rounded-xl border-2 p-3 text-left transition-colors ${
                              selected
                                ? 'bg-purple-500/20 border-purple-400 text-white'
                                : 'bg-zinc-900 border-zinc-700 hover:border-purple-400/70 text-zinc-200'
                            }`}
                          >
                            <p className="font-bold text-sm">{gk.jerseyNumber ? `#${gk.jerseyNumber} ` : ''}{gk.nickname?.trim() || gk.name}</p>
                            <p className="text-[10px] uppercase tracking-wide text-zinc-500 mt-0.5">Goleiro que entrou na partida</p>
                          </button>
                        );
                      }) : (
                        <div className="rounded-xl border border-zinc-700 bg-zinc-900 p-3 text-zinc-400 text-sm">
                    ...
                    <div className="p-3 border-t border-zinc-800 bg-zinc-900/50 flex gap-2 justify-end">
                      ...
                      <button
                        onClick={() => {
                          if (!actionFlow.selectedPlayerId || !actionFlow.details) return;
                          if (needsTimePopup()) {
                            setActionFlow(prev => prev ? { ...prev, step: 'time' } : prev);
                            return;
                          }
                          handleRegisterSave(actionFlow.details as 'simple' | 'hard', actionFlow.selectedPlayerId);
                          cancelActionFlow();
                        }}
                        disabled={!actionFlow.selectedPlayerId}
                        className={`px-4 py-2 font-bold uppercase text-xs rounded-lg transition-colors border ${
                          actionFlow.selectedPlayerId
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-fade-in">
                  ...
                  <div className="relative w-full max-w-md bg-zinc-950 border-2 border-zinc-700 rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        <label className="text-zinc-400 text-xs font-bold uppercase w-20">Período</label>
                        <select
                          value={actionFlow.pendingPeriod ?? '1T'}
                          onChange={(e) => setActionFlow(prev => prev ? { ...prev, pendingPeriod: e.target.value as '1T' | '2T' } : null)}
                          className="bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1.5 text-white text-sm font-bold outline-none focus:border-[#00f0ff]"
                        >
                    ...
                    <div className="p-4 border-t border-zinc-800 bg-zinc-900/50 flex gap-3">
                      <button
                        onClick={() => {
                          ...
                          const period = actionFlow.pendingPeriod ?? '1T';
                          completeActionFlowWithPlayer(pid, time, period);
                        }}
