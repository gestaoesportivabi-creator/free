    ...
    <section aria-labelledby="first-match-onboarding-title" data-testid="first-match-onboarding" className="rounded-2xl border border-[#00f0ff]/35 bg-gradient-to-br from-[#00f0ff]/12 via-zinc-950 to-zinc-950 p-5 sm:p-6 shadow-[0_0_30px_rgba(0,240,255,0.08)]">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 text-sm leading-6 text-zinc-300">Complete o essencial uma vez. Depois, escolha entre coleta em tempo real ou preenchimento p&oacute;s-jogo na tela da partida.</p>
        </div>
      ...
      <ol className="mt-6 grid gap-3 sm:grid-cols-3" aria-label="Etapas para a primeira partida">
        <OnboardingStep icon={Shield} label="Equipe" complete={hasTeam} detail="Defina o nome do time nas configura&ccedil;&otilde;es." />
        <OnboardingStep icon={Users} label="Elenco" complete={playerCount > 0} detail={playerCount > 0 ? `${playerCount} atleta${playerCount === 1 ? '' : 's'} cadastrado${playerCount === 1 ? '' : 's'}.` : 'Cadastre ao menos um atleta para come&ccedil;ar.'} />
